...
def validate_personal_info(processed_docs, personal_data):
    ...
    for doc_type, doc_data in processed_docs.items():
        ...
        for part in name_parts:
            if len(part) > 2 and part in text:  # Only check parts with length > 2 (skip "de", "la", etc.)
                name_matches += 1
...
def validate_academic_info(processed_docs, academic_data):
    ...
    if grade and 'transcript' in processed_docs:
        ...
        grades_in_transcript = re.findall(grade_pattern, transcript_text, re.IGNORECASE)
...
def validate_cross_document_consistency(processed_docs):
    ...
    for doc_type, doc_data in processed_docs.items():
        ...
        if person_entities:
            names_by_doc[doc_type] = person_entities[0]  # Use the first person entity
        else:
    ...
    if len(names_by_doc) >= 2:
        ...
        for doc_type, name in list(names_by_doc.items())[1:]:
            ...
            similarity = difflib.SequenceMatcher(None, reference_name.lower(), name.lower()).ratio()

...
def render_validation_history():
    ...
    if not email:
        st.warning("Please enter your email in the Personal Information step to view your validation history.")
        return
        ...
        with st.expander(f"{status_icon} Validation #{i+1} - {formatted_date}"):
            ...
            with tabs[0]:
                ...
                if validation_results:
                    ...
                    personal_validation = validation_results.get('personal_validation', {})
                    st.subheader("Personal Information Validation")
                    st.write(f"Status: {personal_validation.get('status', 'Unknown')}")
                    ...
                    if issues:
                        ...
                        for issue in issues:
                            ...
                            st.write(f"{icon} {issue.get('description', 'Unknown issue')}")
                    else:
                        st.write("No issues found.")
                    
                    # Display academic validation
                    academic_validation = validation_results.get('academic_validation', {})
                    st.subheader("Academic Information Validation")
                    st.write(f"Status: {academic_validation.get('status', 'Unknown')}")

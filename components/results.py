...
def display_validation_results(validation_results):
    ...
    with col1:
        display_category_result(
            "Personal Information", validation_results['personal_validation']['status'])

    with col2:
        display_category_result(
            "Academic Information", validation_results['academic_validation']['status'])

    with col3:
        display_category_result(
            "Document Authenticity", validation_results['document_authenticity']['status'])

    with col4:
        display_category_result("Cross-Document Consistency",
                                validation_results['cross_document_consistency']['status'])
    ...
    with st.expander("Cross-Document Consistency Validation", expanded=overall_status != 'Passed'):
        display_category_details(
            validation_results['cross_document_consistency'])
    ...
    if overall_status == 'Passed':
        ...
        st.write(
            "Based on the validation results, we recommend the following actions:")
        ...
        if all_issues:
            ...
            st.write(
                "No specific issues found, but some validation checks didn't pass completely.")
    ...
    if status == 'Passed':
        ...
        if issues:
            for issue in issues:
                ...
                if severity == 'critical':
                    st.error(
                        f"❌ **{issue_type}** in {document}: {description}")
                else:
                    st.warning(
                        f"⚠️ **{issue_type}** in {document}: {description}")
        else:
